...
import { Upload, FileSpreadsheet, X, CheckCircle2, AlertCircle, ArrowRight } from 'lucide-react';
  ...
  const handleImport = useCallback(() => {
    ...
    const processBatch = () => {
      ...
      for (let i = idx; i < end; i++) {
        const row = allRows[i];
        try {
          const entry: Partial<Client> = {
            id: String(globalClients.length + imported + 1),
            flags: [],
            mes_referencia: 'Março 2026',
          };
      ...
      if (idx < total) {
        ...
        setResult({ imported, errors: errorDetails.length, errorDetails });
        setStep('done');
      }
    };
    requestAnimationFrame(processBatch);
        ...
        <Card>
          <CardContent className="p-8">
            ...
            >
              ...
              <p className="text-lg font-semibold text-foreground">Arraste seu arquivo CSV aqui</p>
              <p className="text-sm text-muted-foreground">ou clique para selecionar</p>
              <p className="text-xs text-muted-foreground">Formatos aceitos: .csv, .txt (delimitado por vírgula, ponto-e-vírgula ou tab)</p>
            </div>
            <input
              ref={fileRef}
              type="file"
          ...
          <CardContent className="p-8 space-y-4">
            ...
            {result.errors > 0 && (
              <div className="bg-overdue/10 border border-overdue/20 rounded-lg p-4 space-y-1">
                <div className="flex items-center gap-2 text-sm font-medium text-overdue">
